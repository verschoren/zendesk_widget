/**
 * Cloudflare Pages Function for Zendesk Answer Bot - Resolution
 *
 * This endpoint marks an article as resolved in the Answer Bot API.
 *
...
export async function onRequestPost(context: {
  ...
  if (!payload.article_id || !payload.interaction_access_token) {
    return new Response(
      JSON.stringify({
        error: 'Missing required fields',
        required: ['article_id', 'interaction_access_token']
      }),
      {
        status: 400,
        headers: {
  ...
  try {
    ...
    const zendeskUrl = `https://${env.ANSWERBOT_DOMAIN}.zendesk.com/api/v2/answer_bot/resolution`

    // Forward request to Zendesk API
    // Note: Resolution endpoint doesn't require authentication, only the interaction token
    ...
    console.error('Answer Bot resolution error:', error)
    return new Response(
      JSON.stringify({
        error: 'Failed to resolve article',
